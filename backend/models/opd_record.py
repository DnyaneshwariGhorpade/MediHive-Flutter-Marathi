        ...
        with get_db() as db:
            db.execute("""
                ...
                ON CONFLICT (id) DO UPDATE SET
                    opd_id = COALESCE(NULLIF(EXCLUDED.opd_id, ''), opd_visits.opd_id),
                    patient_id = COALESCE(NULLIF(EXCLUDED.patient_id, ''), opd_visits.patient_id),
                    opd_type = COALESCE(NULLIF(EXCLUDED.opd_type, ''), opd_visits.opd_type),
                    symptoms = COALESCE(NULLIF(EXCLUDED.symptoms, ''), opd_visits.symptoms),
                    diagnosis = COALESCE(NULLIF(EXCLUDED.diagnosis, ''), opd_visits.diagnosis),
                    medicines = COALESCE(NULLIF(EXCLUDED.medicines, ''), opd_visits.medicines),
                    visit_datetime = COALESCE(NULLIF(EXCLUDED.visit_datetime, ''), opd_visits.visit_datetime),
                    clinical_notes = COALESCE(NULLIF(EXCLUDED.clinical_notes, ''), opd_visits.clinical_notes),
                    consultation_fee = EXCLUDED.consultation_fee,
                    medicine_fee = EXCLUDED.medicine_fee,
                    panchakarma_fee = EXCLUDED.panchakarma_fee,
                    ...
                    image_links = COALESCE(NULLIF(EXCLUDED.image_links, ''), opd_visits.image_links),
                    panchakarma_notes = COALESCE(NULLIF(EXCLUDED.panchakarma_notes, ''), opd_visits.panchakarma_notes),
                    clinic_id = COALESCE(NULLIF(EXCLUDED.clinic_id, ''), opd_visits.clinic_id),
                    device_id = COALESCE(NULLIF(EXCLUDED.device_id, ''), opd_visits.device_id),
                    sync_status = EXCLUDED.sync_status,
                    last_synced_at = EXCLUDED.last_synced_at,
                    updated_at = EXCLUDED.updated_at
            """, (
                data['id'], data.get('opd_id', ''), data.get('patient_id', ''), data.get('opd_type', 'OPD'),
